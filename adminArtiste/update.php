<?php
    ...
    if(!empty($_POST)) 
    {
        ...
        { 
            ...
            if($isImageUpdated)
            {
                $statement = $db->prepare("UPDATE artise  set nomArtise = ?, presentation = ?,  category = ?, img = ? WHERE id = ?");
                $statement->execute(array($name,$description,$category,$image,$id));
            }
            else
            {
                $statement = $db->prepare("UPDATE artise  set name = ?, presentation = ?, , category = ? WHERE id = ?");
        ...
        {
            $db = Database::connect();
            $statement = $db->prepare("SELECT * FROM artise where id = ?");
    ...
    <body>
        <h1 class="text-logo"></span> Anomic Element </span></h1>
         <div class="container admin">
            <div class="row">
                <div class="col-sm-6">
                    <h1><strong>Modifier un artiste</strong></h1>
                    <br>
                    <form class="form" action="<?php echo 'update.php?id='.$id;?>" role="form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <?php
                               ...
                               {
                                    if($row['id'] == $category)
                                        echo '<option selected="selected" value="'. $row['id'] .'">'. $row['name_cate'] . '</option>';
                                    else
